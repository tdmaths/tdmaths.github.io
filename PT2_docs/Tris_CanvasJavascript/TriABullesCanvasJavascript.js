...
function drawlist(list,j,k,indexj,indexk,x,y,ctx) {
  ...
  if (indexk != "") {
    ...
    ctx.fillStyle = truecolor;
    ctx.beginPath();
    ctx.rect(x*unit+(k+1)*unit,y*skip,unit,unit);
...
function* triABulles(list,x0,x1,x2,y,ctx) {
  var i=0,i1=0,i2=0;
  var u=0, j=0;
  
  while (i < list.length) {
    ctx.clearRect(0,0,canvas.width,canvas.height);
    drawlist(list,list.length - 1 - i,0,"n-1-i","",x1,y+1,ctx);
    yield(1000);
    j=0
    while (j < (list.length - 1 - i) ) {
        ctx.clearRect(0,0,canvas.width,canvas.height);
        drawlist(list,list.length - 1 - i,j,"n-1-i","j",x1,y+1,ctx);
        yield(1500);
    ...
    ctx.clearRect(0,0,canvas.width,canvas.height);
    drawlist(list,list.length - 1 - i,j,"n-1-i","",x1,y+1,ctx);
    yield(1000);
...
function* startanim(ctx) {
  ...
  yield* triABulles(list,x0,x1,x2,topskip/skip,ctx);
}
